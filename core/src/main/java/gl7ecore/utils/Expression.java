...
public class Expression
 {
	...
	public String doTrans(String input)throws Exception{
	    clear();
		if(!isEmpty(input))
		{
			throw new Exception("");
	...
	public float calculate_check(String ii,HashMap<String,Double> vars)throws Exception{
		String[] in=ii.split("·");
		...
		for(int i=0;i<in.length;i++)
		{
				...
				{
					if(vars!=null){
					    if(vars.containsKey(in[i])){
                            st.push(vars.get(in[i]));
                            continue;
                        }
                    }
				if(in[i].equals(","))
				...
				{if(in[i].equals("arcsin"))
					...
					st.push(Double.valueOf(in[i]));}
		if(st.size()!=1){throw new Exception("exp check error");}
		return (float)(double)st.pop();
	}
	public float calculate(String ii,HashMap<String,Double> vars){
